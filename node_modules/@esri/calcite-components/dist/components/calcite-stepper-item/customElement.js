...
import { c as componentFocusable, i as isHidden } from "../../chunks/component.js";
...
class StepperItem extends LitElement {
  ...
  render() {
    ...
    return InteractiveContainer({ disabled: this.disabled, children: html`<div class=${safeClassMap(CSS.container)}>${this.complete && html`<span aria-live=polite class=${safeClassMap(CSS.visuallyHidden)}>${this.messages.complete}</span>` || ""}<div class=${safeClassMap(CSS.stepperItemHeader)} tabindex=${innerDisplayContextTabIndex ?? nothing} ${ref(this.headerEl)}>${this.icon ? this.renderIcon() : null}${this.numbered ? html`<div class=${safeClassMap(CSS.stepperItemNumber)}>${this.renderNumbers()}.</div>` : null}<div class=${safeClassMap(CSS.stepperItemHeaderText)}><span class=${safeClassMap(CSS.stepperItemHeading)}>${this.heading}</span><span class=${safeClassMap(CSS.stepperItemDescription)}>${this.description}</span></div></div><div class=${safeClassMap(CSS.stepperItemContent)}><slot></slot></div></div>` });
  }
  renderIcon() {
    let path = "circle";
    ...
    return html`<calcite-icon class="stepper-item-icon" .flipRtl=${this.iconFlipRtl} .icon=${path} scale=s></calcite-icon>`;
  }
  renderNumbers() {
    numberStringFormatter.numberFormatOptions = {
